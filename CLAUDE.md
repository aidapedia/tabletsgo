...
All metadata-schema evolution lives in `initMetaDb()` in `server.js` and runs idempotently on every boot — there are no migration files. Follow these rules so in-app updates (and rolling back to an older image) stay safe:

- **New table** ⇒ add a `CREATE TABLE IF NOT EXISTS` block. **New column** ⇒ add an `addColumn('table', 'col TYPE')` call (probe-then-`ALTER TABLE ADD COLUMN`; never edit an existing `CREATE TABLE` block for old installs — it only applies to fresh DBs). **Backfills** must be idempotent and guarded (e.g. `WHERE x IS NULL`, `type IS NULL AND data IS NOT NULL`) so re-running on an already-migrated DB is a no-op.
...
- **`META_SCHEMA_VERSION`** (persisted via `PRAGMA user_version`) gates the pre-migration snapshot: when the persisted version is behind the code's on an existing install, the boot copies the meta DB to `data/backups/pre-migrate-v{N}-{ts}.db` before migrating. **Bump it whenever a migration is risky enough to deserve that snapshot** (backfills that rewrite rows, credential/format changes); routine `addColumn`s don't require a bump.
- Precedents to copy: the pre-workspace → Default Workspace migration and the plaintext-`data` → encrypted-`credentials` backfill, both in `initMetaDb()`.
