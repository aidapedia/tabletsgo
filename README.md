...
| `SESSION_REDIS_URL` | | Keeps sessions (logins + open database connections) in Redis instead of in‑process. `docker-compose.yml` points this at its bundled `redis` service, so logins survive a restart and multiple replicas share them. Clear it to use the in‑process store (single container; a restart signs everyone out). |
