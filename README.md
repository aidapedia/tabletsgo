...
| 👥 | **Workspaces & teams** | Multi‑workspace (org/tenant) model with members, teams and email invites (SMTP optional — always get a copyable link). Roles come in two tiers: an **instance admin** manages which workspaces exist and who runs them (and deliberately has no data access of their own), while inside a workspace people hold a **configurable role**. |
| 🛡️ | **Roles & permissions (RBAC)** | Workspace roles are **defined by an admin**, not hardcoded. Under **Administration → Roles**, build a role out of nine permissions — manage the workspace, delete it, manage members, teams, notifications, storage destinations, add connections, manage all connections, transfer connection ownership — and workspace owners assign their people to it. `Owner` and `Member` ship built‑in and can be renamed or re‑permissioned; a workspace always keeps at least one member who can manage it. Separately, **every connection has an owner**: whoever owns one can edit, back up and delete it whatever their role, and ownership can be handed to another member. |
