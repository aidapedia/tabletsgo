...
| 🌳 | **Resource tree** | One hierarchy of everything on the instance, under **Browse → Resource Tree**: **Application** at the root, workspaces under it, then your own **groups**, and the connections, storage destinations, dashboards and workflows filed inside them. It's not just navigation — it's where access lives. Pick any node to see who owns it, who's been granted a role on it, and exactly what *you* can do there. Make a group like "Production" by right‑clicking wherever it belongs in the tree, move connections into it (right‑click → **Move…**, or the move button in the node panel — the picker only offers places that can hold it and that you may file into), and grant someone a role on that group alone. Moving re‑files nothing about access itself: the node simply starts inheriting whatever is granted on its new parent. A group is also a set of **people**: put a few members in it and every grant made to that group reaches all of them — so access is granted once, not per person. (Nesting a group under another only files resources; it never merges who's in them.) |
