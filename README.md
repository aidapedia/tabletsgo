...
[![Version](https://img.shields.io/badge/version-0.20.0-6FCF6A)](package.json)
...
| 🔑 | **Auth & security** | First‑run setup wizard, token‑based auth, two‑tier roles, and membership‑guarded connection routes. Logins are stored in the metadata DB and read through a cache, so a restart — or a Redis outage — doesn't sign anyone out. |
...
| `SESSION_REDIS_URL` | | Makes the session **cache** Redis instead of in‑process. Logins live in the metadata DB either way, so clearing this never signs anyone out; what Redis adds is replicas sharing one cache and seeing each other's open database connections (those are cache‑only, since they track live sockets). Required for more than one replica; `docker-compose.yml` points it at its bundled `redis` service. |
...
| `MAX_SESSIONS_PER_CONNECTION` | | Instance‑wide default cap on concurrent sessions per connection (`0` = unlimited). A workspace (Workspace → General) or an individual connection can override it. |
