...
| 🔑 | **Auth & security** | First‑run setup wizard, token‑based auth, two‑tier roles, and membership‑guarded connection routes. Logins are stored in the metadata DB and read through an in‑process cache, so a restart doesn't sign anyone out. |
...
| `MAX_SESSIONS_PER_CONNECTION` | | Instance‑wide default cap on concurrent sessions per connection (`0` = unlimited). A workspace (Workspace → General) or an individual connection can override it. A session tracks a database handle the process holds, so with more than one replica the cap applies per replica. |
