...
let addId = 0
const newColumn = (type) => ({ id: `a${++addId}`, name: '', type, notNull: false, default: '' })

export default function TableEditPanel({ table, dialect, types, onStage, onClose }) {
  ...
  const buildStatements = () => {
    ...
    for (const c of added) {
      ...
      let s = `ALTER TABLE "${t}" ADD COLUMN "${c.name.trim()}" ${c.type}`
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="mt-2 flex flex-col gap-3">
            {added.map((c) => (
              <div key={c.id} className="rounded-soft border border-edge bg-elevated/40 p-2.5">
                <div className="flex items-center gap-2">
                  <input
                    className={`${fieldInput} !w-auto min-w-0 flex-1`}
                    placeholder="column_name"
                    value={c.name}
                    onChange={(e) => setAd(c.id, { name: e.target.value })}
                  />
                  <Select
                    className={`${fieldInput} !w-[120px] shrink-0`}
                    value={c.type}
                    onChange={(v) => setAd(c.id, { type: v })}
                    options={types.map((t) => ({ value: t, label: t }))}
                  />
                  <button
                    type="button"
                    className="shrink-0 text-ink-dim hover:text-red"
                    onClick={() => removeAdded(c.id)}
                    aria-label="Remove"
                  >
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <input
                    ...
                    placeholder="default…"
                    value={c.default}
                    onChange={(e) => setAd(c.id, { default: e.target.value })}
                  />
                </div>
              </div>
            ))}
